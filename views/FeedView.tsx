            ...
            >
              ...
              <div className="flex-1">
                <div className="flex items-center gap-3 mb-3">
                  ...
                  {alert.verificationStatus === 'AI-synthesized' && (
                    <span className="px-2 py-0.5 rounded-md bg-slate-500/10 text-slate-400 text-[8px] font-black uppercase tracking-widest flex items-center gap-1">
                      AI-synthesized
                    </span>
                  )}
                ...
                <div className="mb-6 space-y-2">
                  ...
                  <div className="p-3 bg-blue-500/5 rounded-xl border border-blue-500/10">
                    <p className="text-[9px] text-slate-500 italic leading-snug">
                      {alert.severity === 'High' 
                        ? `Evidence: ${alert.title} at ${alert.location}. ${alert.confidence ? `Signal confidence: ${alert.confidence}%.` : ''} Direct proximity match with monitored supply chain node.`
                        : alert.severity === 'Medium'
                          ? `Evidence: ${alert.title} at ${alert.location}. ${alert.confidence ? `Signal confidence: ${alert.confidence}%.` : ''} Moderate impact expected on regional logistics.`
                          : `Evidence: Standard operational telemetry from ${alert.location}. No active disruption signals detected.`}
                    </p>
                  </div>
                </div>
              </div>
