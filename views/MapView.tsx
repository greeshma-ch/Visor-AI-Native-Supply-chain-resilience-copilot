...
const MapView: React.FC<MapViewProps> = ({ suppliers, categoryFilter, statusFilter, onSelectSupplier, hqLocation, disruptions = [] }) => {
  ...
  const categorySuppliers = React.useMemo(() => suppliers.filter(s => {
  ...
  const chartData = React.useMemo(() => {
    ...
    const total = categorySuppliers.length;
    return [
      { name: 'Stable', value: Math.round((stable / total) * 100), color: '#10b981', id: RiskStatus.STABLE },
      { name: 'Caution', value: Math.round((caution / total) * 100), color: '#f59e0b', id: RiskStatus.CAUTION },
      { name: 'Risky', value: Math.round((risky / total) * 100), color: '#f43f5e', id: RiskStatus.RISKY },
    ].filter(d => d.value > 0).map(item => ({
      ...item,
      color: statusFilter === 'ALL' || statusFilter === item.id ? item.color : `${item.color}44`
    }));
  }, [categorySuppliers, statusFilter]);
    ...
    <div className="flex flex-col xl:flex-row h-full animate-in fade-in duration-700 bg-[#070b14] overflow-hidden">
      ...
      <div className={`${isPanelCollapsed ? 'xl:w-16' : 'xl:w-96'} w-full bg-[#070b14]/60 backdrop-blur-2xl border-t xl:border-t-0 xl:border-l border-white/10 flex flex-col transition-all duration-500 ease-in-out shadow-2xl flex-1 xl:flex-none xl:h-full overflow-hidden relative`}>
        ...
        <div className={`p-4 sm:p-8 space-y-6 sm:space-y-8 flex-1 overflow-y-auto custom-scrollbar transition-all duration-300 ${isPanelCollapsed ? 'xl:hidden pointer-events-none' : 'opacity-100'}`}>
          ...
          <div className="relative w-full flex items-center justify-center mt-4" style={{ height: 220 }}>
             ...
             <div className="absolute inset-0 flex flex-col items-center justify-center pt-2 pointer-events-none">
                <span className="text-3xl sm:text-4xl font-black text-white tracking-tighter block leading-none">{filteredSuppliers.length}</span>
                <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest block mt-1">Active Nodes</span>
             </div>
          </div>
