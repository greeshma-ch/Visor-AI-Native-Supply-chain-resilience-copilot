...
const DashboardView: React.FC<DashboardViewProps> = ({ 
  ...
  const cautionRiskySupplierIds = React.useMemo(() => new Set(
    channelSuppliers.filter(s => s.status === RiskStatus.CAUTION || s.status === RiskStatus.RISKY).map(s => s.id)
  ), [channelSuppliers]);

  const filteredDisruptions = React.useMemo(() => {
    const targetSuppliers = channelSuppliers.filter(
      s => s.status === RiskStatus.CAUTION || s.status === RiskStatus.RISKY
    );

    return disruptions.filter(d => {
      return targetSuppliers.some(s => {
        const isDirectMatch = d.impactedSuppliers.includes(s.id) || d.impactedSuppliers.includes(s.name);
              ...
              return (
                ...
                >
                  <motion.button 
                    ...
                    className="w-full text-left p-5 rounded-[2rem] bg-white/[0.02] border border-white/5 hover:border-white/10 transition-all group flex flex-col justify-start gap-3 min-h-[150px]"
                  >
                    <div className="flex flex-row items-start justify-between w-full gap-2 flex-wrap">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="px-2 py-0.5 rounded-md bg-blue-500/10 text-blue-400 text-[8px] font-black uppercase tracking-widest whitespace-nowrap">
                          {d.type}
                        </span>
                        {d.verificationStatus === 'verified' && (
                          <span className="px-2 py-0.5 rounded-md bg-emerald-500/10 text-emerald-400 text-[8px] font-black uppercase tracking-widest whitespace-nowrap">
                            Live
                          </span>
                        )}
                        {d.verificationStatus === 'AI-synthesized' && (
                          <span className="px-2 py-0.5 rounded-md bg-slate-500/10 text-slate-400 text-[8px] font-black uppercase tracking-widest whitespace-nowrap">
                            AI-synthesized
                          </span>
                        )}
                      </div>
                      <span className="text-[8px] font-black text-slate-600 uppercase tracking-widest flex items-center gap-1 flex-shrink-0">
                        <Clock size={10} /> {getRelativeTime((d as any).lastUpdated || d.timestamp)}
                      </span>
                    </div>
                    <h4 className="font-semibold text-white text-sm sm:text-base leading-tight group-hover:text-blue-400 transition-colors uppercase line-clamp-2">{displayTitle}</h4>
