...
import { checkGroqConnection } from '../services/apiClient';
...
const SettingsView: React.FC<SettingsViewProps> = ({ user, onLogout }) => {
  ...
  const [connectionStatus, setConnectionStatus] = useState<{ success: boolean; message: string; modelUsed?: string } | null>(null);
    ...
    try {
      ...
      if (result.success) {
        toast.success("Connection Verified", {
          description: "Groq connection verified successfully."
        });
      } else {
  ...
  return (
          ...
          <div className="bg-[#0a0f1c] p-6 sm:p-10 rounded-[2rem] sm:rounded-[2.5rem] border border-white/5 shadow-2xl space-y-8 sm:space-y-10">
            ...
            <div className="space-y-8 sm:space-y-10">
              ...
              <div className="pt-6 border-t border-white/5 flex flex-col gap-4">
                <div className="flex items-center justify-between gap-6">
                  <div>
                    <p className="text-sm font-extrabold text-white uppercase tracking-tight">Engine Connection</p>
                    <p className="text-[10px] text-slate-500 font-medium mt-1 uppercase tracking-widest">Verify Groq connection status.</p>
                  </div>
                  <button 
                    onClick={handleCheckConnection}
                    disabled={isCheckingConnection}
                    className="px-4 py-2 bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-white text-[9px] font-black uppercase tracking-widest rounded-xl transition-all shadow-lg shadow-blue-900/40"
                  >
                    {isCheckingConnection ? 'Verifying...' : 'Verify'}
                  </button>
                </div>
                {connectionStatus && (
                  <div className={`p-3 rounded-xl border text-[10px] font-bold uppercase tracking-wider flex items-center gap-2 animate-in fade-in slide-in-from-top-1 duration-200 ${
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="font-extrabold truncate">{connectionStatus.message}</p>
                      {connectionStatus.modelUsed && (
                        <p className="text-[8px] text-slate-500 mt-0.5 lowercase">model: {connectionStatus.modelUsed}</p>
                      )}
                    </div>
                  </div>
                )}
    ...
    <div className="flex justify-center pt-6">
      <button onClick={onLogout} className="px-8 py-3 bg-rose-600/10 border border-rose-500/20 text-rose-500 font-black uppercase tracking-widest text-[10px] rounded-xl hover:bg-rose-600 hover:text-white transition-all">
        Terminate Session
      </button>
    </div>
  </div>
